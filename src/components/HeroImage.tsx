...
export function HeroImage({ currentMonth, accentColor, onNext, onPrev }: HeroImageProps) {
  ...
  return (
    <div className="relative w-full h-[400px] flex-shrink-0 overflow-hidden bg-neutral-100 dark:bg-neutral-800 rounded-t-sm z-10">
      <div 
      ...
      <div className="absolute top-6 right-6 z-20 flex space-x-2">
        ...
        <button
          ...
          className="p-2 rounded-full bg-black/20 hover:bg-black/40 text-white backdrop-blur-md transition-colors w-9 h-9 flex items-center justify-center"
      ...
      <div className="absolute bottom-0 inset-x-0 z-10 transform translate-y-[2px]">
        ...
        <svg viewBox="0 0 1440 320" className="w-full text-white dark:text-neutral-900 transition-colors duration-500" preserveAspectRatio="none" style={{ height: '140px' }}>
          {/* Accent colored angled band behind the wave */}
          <path 
            fill={accentColor}
            fillOpacity="1" 
        ...
        {/* Text inside the blue shape, positioned absolutely relative to bottom of container */}
        <div className="absolute bottom-8 right-12 z-20 text-white text-right drop-shadow-md">
          <div className="text-2xl font-light tracking-widest">{format(currentMonth, 'yyyy')}</div>
          <div className="text-4xl sm:text-5xl font-bold uppercase tracking-wider">{format(currentMonth, 'MMMM')}</div>
        </div>
      </div>
    </div>
  );
